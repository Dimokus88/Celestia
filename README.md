# Stride validator node on Akash Network
# Нода валидатора сети Stride, развертка в Akash Network.
<div align="center">
...
| [Discord Stride](https://discord.gg/MZkw6wHwnF) | [Explorer Stride](https://poolparty.stride.zone/STRIDE) | [Site Stride](https://stride.zone/) | [Twitter Stride](https://twitter.com/stride_zone) |
|:--:|:--:|:--:|:--:|
  
</div>
<div align="center">
  
[English version](https://github.com/Dimokus88/Stride#english-version) | [Русская версия](https://github.com/Dimokus88/Stride#%D1%80%D1%83%D1%81%D1%81%D0%BA%D0%B0%D1%8F-%D0%B2%D0%B5%D1%80%D1%81%D0%B8%D1%8F)
  
[Node management commands | Команды управления нодой](https://github.com/Dimokus88/Stride/blob/main/README.md#node-management-commands-via-ssh--%D0%BA%D0%BE%D0%BC%D0%B0%D0%BD%D0%B4%D1%8B-%D1%83%D0%BF%D1%80%D0%B0%D0%B2%D0%BB%D0%B5%D0%BD%D0%B8%D1%8F-%D0%BD%D0%BE%D0%B4%D0%BE%D0%B9-%D1%87%D0%B5%D1%80%D0%B5%D0%B7-ssh)
  
</div>

# English version

> If you want to migrate your Stride node to Akash, or if you have priv_validator_key.json, then go [to this step](https://github.com/Dimokus88/Stride#if-you-have-priv_validator_keyjson).
...
***Create an additional Cosmos ecosystem wallet for the Stride project using Keplr or Cosmostation. Rewrite the seed phrase from the created wallet, we will need it when deploying.***
...
* Click ***CREATE DEPLOYMENT***. Select ***Empty*** and copy the contents there [deploy.yml](https://github.com/Dimokus88/Stride/blob/main/deploy.yml)
...
* ```MONIKER```       - Node name .
* ```MNEMONIС```      -  insert the mnemonic phrase from your wallet ***Stride***.

> ```LINK_KEY``` -  comment out the env to the priv_validator_key.json. 

In the ```resources``` field, we set the capacity to be rented. ```2 CPU x 3 GB RAM x 40 GB SSD```  for ***Stride*** node. 
...
* In the ```SHELL``` tab, run the command```cat /root/.stride/config/priv_validator_key.json```, save the answer in a file```priv_validator_key``` with extension```.json```.
...
*In the process of work, your address ***Stride*** will be displayed, you need to request tokens to it in [Discord Stride](https://discord.gg/MZkw6wHwnF).
...
[Go to start](https://github.com/Dimokus88/Stride#Stride-validator-node-on-akash-network)
...
In the ```resources``` field, we set the capacity to be rented. ```2 CPU x 3 GB RAM x 40 GB SSD``` recommended for ***Stride*** node. 
...
> Если хотите перенести вашу ноду на Akash, или у вас есть priv_validator_key.json, то перейдите [к этому пункту](https://github.com/Dimokus88/Stride#%D0%B5%D1%81%D0%BB%D0%B8-%D1%83-%D0%B2%D0%B0%D1%81-%D0%B5%D1%81%D1%82%D1%8C-priv_validator_keyjson).

> На вашем кошельке ```Akash``` (с которого будет разворачивать ***Stride***) должно быть более ***5 АКТ*** (5 АКТ будут заблокированы на развертывание + оплата газа транзакций). АКТ можно пробрести на биржах ```Gate```, ```AsendeX```, ```Osmosis``` . Так же в нашем сообществе [Akash RU](https://t.me/akash_ru) мы регулярно проводим эвенты в которых раздаем АКТ.

## Если запуск производится впервые:

***Создайте дополнительный кошелек экосистемы Cosmos для проекта Stride, с помощью Keplr или Cosmostation. Перепишите seed фразу от созданного кошелька, она понадобится нам при развертке.***
...
* Нажимаем ```CREATE DEPLOYMENT```. Выбираем ```Empty```(пустой template) и копируем туда содержимое [deploy.yml](https://github.com/Dimokus88/Stride/blob/main/deploy.yml) .
...
В поле ***MNEMONIС*** - вставляем мнемоник фразу от вашего кошелька ***Stride***.

> Поле ***LINK_KEY*** -  оставьте закомментированным ссылка на размещенный priv_validator_key.json (прямое скачивание).

Ниже, в поле ```resources``` мы выставляем арендуюмую мощность. для ноды ***Stride*** рекомендуется ```2 CPU x 3 GB RAM x 40 GB SSD```.  
...
* Во вкладке ```SHELL``` выполните команду ```cat /root/.stride/config/priv_validator_key.json```, ответ сохраните в файле ```priv_validator_key``` с расширением ```.json```.
...
* В процессе работы будет выводится ваш адрес ***Stride***, на него нужно запросить токены в [Discord Stride](https://discord.gg/MZkw6wHwnF).
...
[Перейти к началу](https://github.com/Dimokus88/Stride#Stride-validator-node-on-akash-network)
...
Ниже, в поле ```resources``` мы выставляем арендуемую мощность. для ноды ***Stride*** рекомендуется ```2 CPU x 3 GB RAM x 40 GB SSD```.
